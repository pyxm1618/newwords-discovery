...
For real-time requests, call the MCP tool `get_trending_now` once for each requested country.
...
Use the MCP tool `get_trending_keywords` with `kind=rising` for daily discovery/backfill and rolling weekly history.
